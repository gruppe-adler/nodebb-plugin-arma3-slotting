...
module.exports.notifySlotted = function (match, slotUuid, oldUser, newUser) {
    ...
    async.parallel({
        ...
        let msg = 'User %s slotted into "%s"'.replace('%s', newUser.username).replace('%s', eventTitle);
        if (oldUser) {
...
module.exports.notifyUnslotted = function (match, slotUuid, oldUser) {
    ...
    async.parallel({
        ...
        const eventTitle = data.eventTitle;
        const followingUids = data.followingUids;
        let msg = 'User %s slotted out of "%s"'.replace('%s', oldUser.username).replace('%s', eventTitle);
        notifications.create({
            bodyShort: msg,
            bodyLong: msg,
