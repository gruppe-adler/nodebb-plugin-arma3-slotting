...
function putMatch(tid, matchId, res, reqBody, callback) {
    ...
    function saveUsers(xmlDoc, callback) {
        xmlDoc.find('//user').forEach(function (user) {
            const slot = user.parent();
            if (!slot || slot.name() !== 'slot') {
                return callback(new Error('found user node with non-slot parent ' + slot));
            }
            const uid = user.attr('uid');
            if (!uid) {
                return callback(new Error('user without uid found!'));
            }

            const slotid = slot.attr('uuid');
            slotDb.putMatchUser(tid, matchId, slotid, uid, callback);
        });
    }
    function saveReservations(xmlDoc, callback) {
        xmlDoc.find('//reservation').forEach(function (reservation) {
            ...
            const slotid = slot.attr('uuid');
            slotDb.putMatchUser(tid, matchId, slotid, reservation.text, callback);
        });
    }
