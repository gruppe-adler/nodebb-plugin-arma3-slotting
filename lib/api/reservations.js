...
module.exports.put = function (req, res, next) {
    ...
    slotDb.putMatchReservation(tid, matchid, slotid, model['reserved-for'], function (err, result) {
        if (err) {
            return res.status(500).json(err);
        }
        winston.info('reservation put for match %s, slot %s'.replace('%s', matchid).replace('%s', slotid));
